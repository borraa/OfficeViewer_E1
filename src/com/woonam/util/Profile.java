...
public class Profile 
{
	...
	public Profile(String strConfPath) 
	{
		...
		try
		{
			...
			if(br != null)
			{
				...
				while(line != null)
				{
					 ...
					 * save value item
					 */
					if(!m_C.isBlank(strHeader)) putValueItem(mapValue, line);
				 ...
				 */
				if(!m_C.isBlank(strHeader) && !mapValue.isEmpty())
				{
					mapRes.put(strHeader, mapValue);
				}
	...
	private void putValueItem(HashMap<String, String> mapValue, String strLine)
	{
		 ...
		 */
		if(strLine.indexOf(commentSign) > -1)
		{
			return;
		}
    ...
    public String getString(String strSection, String strKey, String strDefault)
    {
    	...
    	if(m_C.isBlank(strRes)) strRes = strDefault;
