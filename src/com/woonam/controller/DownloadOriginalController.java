...
@WebServlet(urlPatterns = {"/DownloadOriginal.do"})
public class DownloadOriginalController extends HttpServlet {
	...
	public DownloadOriginalController() {
		this.m_C 					= new Common();
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String orgIRN 			= req.getParameter("ORG_IRN");
		...
		try
		{
			...
			String fileName = obj_ORG.get("ORG_FILE").getAsString();
			String docIRN = obj_ORG.get("DOC_IRN").getAsString();
			
			if(m_C.isBlank(fileName) || m_C.isBlank(docIRN))
			{
				resp.getWriter().print(m_C.writeResultMsg("F", "FAILED_GET_FILE_INFO"));
				return;
			}
