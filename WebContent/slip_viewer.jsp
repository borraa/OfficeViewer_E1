...
$(function(){
	...
	/* var XPIParams = {
			LOCAL_WAS_URL		: "<c:out value="${LOCAL_WAS_URL}" />",	
			LOCALIZE_PATH 		: "<c:url value ='/global/lang.json' />",
			LANG 						: "<c:out value="${mParams['Lang'][0]}" />",
			SERVER_KEY 			: "<c:out value="${SERVER_KEY}" />",	
			XPI_URL					: "<c:url value='/js/localWAS/OfficeXPI.js' />",
	};
	
	var ActorParams = {
			COMMAND_URL		: "<c:url value='/ActorCommand.do' />",
			ROOT_URL				: "<c:url value='/' />",
			LOCALIZE_PATH 		: "<c:url value ='/global/lang.json' />",
			UISET_PATH			: "<c:url value='/UI/COLOR.json' />",
			CORP_NO 				: "<c:out value="${sessionScope.CORP_NO}" />", 
			USER_ID 				: "<c:out value="${sessionScope.USER_ID}" />", 
			PART_NO 				: "<c:out value="${sessionScope.PART_NO}" />", 
			SERVER_KEY 			: "<c:out value="${SERVER_KEY}" />",	
			LANG 						: "<c:out value="${mParams['Lang'][0]}" />",
			JDOC_NO 				: "<c:out value="${mParams['JDocNo'][0]}" />",
			MENU_PATH				: "<c:url value ='/UI/menu/ACTOR.json' />",
			VIEW_MODE			: "<c:url value ="${mParams['ViewMode'][0]}" />",
	}
	
	$.Actor.init(ActorParams, XPIParams); */
